...
public class JokeAPI : MonoBehaviour
{
    ...
    [SerializeField] private Button _fetchAIResponseButton;


    // API endpoint we'll be calling to get random jokes
    private const string API_URL = "https://api.chucknorris.io/jokes/random";

    private const string HUGGING_FACE_API = "https://api-inference.huggingface.co/models/google/gemma-2-2b-it";
    private const string HUGGING_FACE_TOKEN = "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"; // Replace with your token
    ...
    private async Task FetchAPI(string url, string postData = null)
    {
        using UnityWebRequest request = postData == null ? UnityWebRequest.Get(url) : new UnityWebRequest(url, "POST");
        ...
        if (postData != null)
        {
            ...
            request.SetRequestHeader("Authorization", $"Bearer {HUGGING_FACE_TOKEN}");
        }
    }

    public async Task FetchAIResponse()
    {
        string requestBody = "{\"inputs\": \"Can you please let us know more details about your \"}";
        await FetchAPI(HUGGING_FACE_API, requestBody);
    }
    /// <summary>
    ...
    public void OnButtonClick_FetchAIResponse()
    {
        _ = FetchAIResponse();
    }
